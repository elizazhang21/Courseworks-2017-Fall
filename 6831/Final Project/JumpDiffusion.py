...
def Jump_Diffusion_Process_At_Fixed_Dates(S, maturity, n, mu, sigma, d, Lambda, a, b):
    ...
    return PriceList
'''
p = Jump_Diffusion_Process_At_Fixed_Dates(20, 1, 252, 0.05, 0.2, 0, 100, 0, 0)

time = np.linspace(0, 1, 252)
fig = plt.figure()
ax = plt.subplot(111)
ax.plot(time, p, 'k--', color='blue', label='Sample Path')
...
plt.title('Jump Diffustion Process At Fixed Dates', fontsize=8)
plt.show()
'''
